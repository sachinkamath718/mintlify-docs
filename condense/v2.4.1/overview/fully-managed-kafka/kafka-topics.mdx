...
The page also provides visibility into topic health and operational status, making it easier to administer Kafka topics across environments.
...
| Messages | Opens the message viewer for browsing records. |
| Edit | Updates the topic configuration. |
| Delete | Permanently removes the topic from the Kafka cluster. |

![](/images/condense/v2.4.0/fully-managed-kafka/rwKRGO3QthZ6EMqqYblg%2Fblobs%2FdOKzjOU7T84NTPzYWidw%2Fimage.png)

Topics that are no longer required can be deleted from the Kafka cluster.**Delete** → Permanently deletes the selected topic from the cluster.

<Danger>
  **Warning**

  Deleting a Kafka topic permanently removes the topic and its stored data. This action cannot be undone.
</Danger>
...
Existing topic configurations can be modified using the **Edit** action available from the Topic Details page.

After updating the required configuration values, review the changes before saving them.
