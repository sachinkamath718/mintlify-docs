...
1. **After how much time is a command marked as timed out?** A command is marked as **Timed Out** if it remains in the _Executing stage_ for **more than 60 minutes**. This applies only if the device is online during this period.
2. **Do pre-built and custom commands follow the same timeout rules?** Yes, the **60-minute timeout** applies uniformly to both pre-built and custom commands in the _Executing stage_.
...
6. **What are the stages for command execution?** Commands typically transition through: **Queued** → **Executing** → **Completed**/**Timed Out**/**Failed**.
